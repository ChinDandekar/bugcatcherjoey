...
import { ShieldAlert, Zap, Box, BrainCircuit, Loader2 } from 'lucide-react';
...
export default function Simulation({ initialTeam }) {
    ...
    useEffect(() => {
        ...
        const runSimulation = async () => {
            try {
                const response = await fetch('https://bugcatcherjoey1996--pokemon-ai-optimizer-v2-fastapi-app-dev.modal.run/simulate', {
                    method: 'POST',
                    ...
                    for (const line of lines) {
                        if (line.startsWith('data: ')) {
                            try {
                                ...
                                if (!isMounted) return; // Prevent background hijacks
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '80vh', gap: '2rem' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '2rem' }}>
                ...
                {!cycle ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem', minHeight: '300px' }}>
                        ...
                        <div style={{ position: 'relative', width: '120px', height: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <motion.div
                                ...
                                style={{ position: 'absolute', inset: 0, border: '2px dashed var(--primary)', borderRadius: '50%', opacity: 0.5 }}
                            />
                            <motion.div
                                animate={{ rotate: -360, scale: [1, 1.1, 1] }}
                                transition={{ rotate: { repeat: Infinity, duration: 12, ease: "linear" }, scale: { repeat: Infinity, duration: 3, ease: "easeInOut" } }}
                                style={{ position: 'absolute', inset: 10, border: '2px dotted var(--warning)', borderRadius: '50%', opacity: 0.7 }}
                            />
                            ...
                            <motion.div
                                ...
                                style={{ background: 'var(--bg-dark)', borderRadius: '50%', padding: '1rem', border: '1px solid var(--primary)', boxShadow: '0 0 20px rgba(129, 140, 248, 0.4)' }}
                            >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
                            <motion.span
                                ...
                                style={{ fontSize: '1.4rem', fontFamily: 'var(--font-heading)', color: 'var(--primary)', letterSpacing: '2px', textShadow: '0 0 10px rgba(129, 140, 248, 0.5)' }}
                            >
                                UPLOADING ROSTER TO METAGAME BRAIN
                            </motion.span>
                            <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>
                                INITIALIZING SMOGON DAMAGE CALCULATORS...
                            </span>
                        </div>
                    </div>
                ) : (
                    ...
                    <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem', paddingRight: '1rem' }}>
                        {simulationData.cycles.map((hist, idx) => (
                            <motion.div
                                ...
                                style={{ background: 'var(--glass-bg)', padding: '1rem', borderRadius: '12px', borderLeft: `3px solid ${hist.changes.some(c => c.replace_with && c.replace_with !== c.name) ? 'var(--danger)' : 'var(--primary)'}` }}
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>CYCLE {hist.num}</span>
                                    <span style={{ fontSize: '0.8rem', color: hist.changes.some(c => c.replace_with && c.replace_with !== c.name) ? 'var(--danger)' : 'var(--primary)', fontWeight: 'bold' }}>STRATEGY CHANGE</span>
                                </div>
                                ...
                                {hist.changes.map((c, i) => (
                                    <div key={i} style={{ background: 'rgba(52, 211, 153, 0.1)', color: 'var(--success)', padding: '0.5rem 1rem', borderRadius: '4px', fontSize: '0.9rem', display: 'inline-block', marginBottom: '0.2rem' }}>
                                        {c.replace_with && c.replace_with !== c.name ? `Replaced ${c.name} with ${c.replace_with}` : `Adjusted ${c.name}`}
                                    </div>
                                ))}
                            </motion.div>
                        ))}
